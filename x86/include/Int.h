...
public:
  ...
  // If hex is invalid, returns -1, valid with 0x or without
  int SetBase10(char *value);

  // If hex is invalid, returns -1, valid with 0x or without  
  int SetBase16(char *value);
  
  // If charset is invalid, returns -1, valid with 0x or without
  int SetBaseN(int n,char *charset,char *value);

  void SetByte(int n,unsigned char byte);
  void SetDWord(int n, uint32_t b);
  void SetQWord(int n,uint64_t b);
  void Rand(int nbit, unsigned char *seed);
  void Rand(Int *randMax, unsigned char *seed);
