...
// SHA-256 macros using AVX2 intrinsics
...
void sha256_avx2_transform(__m256i* state, const uint8_t* data[8]) {
    ...
    // Message schedule (message expansion) W[16..63]
    for (int t = 16; t < 64; ++t) {
...
void sha256avx2_8B(
    ...
    size_t lengths[8] = { len0, len1, len2, len3, len4, len5, len6, len7 };

    // Initialize the state with the initial hash values
    sha256_avx2_initialize(state);
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        // Copy input data
        memcpy(padded[i], data[i], lengths[i]);
    ...
    for (size_t j = 0; j < blocks; j++) {
        const uint8_t* block_ptrs[8];
        for (int i = 0; i < 8; ++i) {
            block_ptrs[i] = padded[i] + j * 64;
        }
        sha256_avx2_transform(state, block_ptrs);
    }
    ...
    for (int i = 0; i < 8; ++i) { // For each hash
        unsigned char* hash = hashArray[i];
        for (int j = 0; j < 8; ++j) { // For each state variable
            uint32_t word = digest[j][i]; // digest[state_index][element_index]
#ifdef _MSC_VER
