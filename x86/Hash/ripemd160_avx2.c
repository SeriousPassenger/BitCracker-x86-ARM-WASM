...
// Macro definitions for each operation in the rounds
...
// Macro to load words from the message blocks
...
void ripemd160_avx2_initialize(__m256i *s) {
    memcpy(s, _init, sizeof(_init));
}

// Transform function processes one block for each message
void ripemd160_avx2_transform(__m256i *s, uint8_t *blk[8]) {
...
// Main function to compute RIPEMD-160 hash for 8 messages of arbitrary size
void ripemd160avx2_32(
    ...
    size_t len4, size_t len5,
    size_t len6, size_t len7,
    ...
    unsigned char *d6, unsigned char *d7)
{
    ...
    // Initialize state
    ripemd160_avx2_initialize(s);

    // Find maximum padded length
    size_t max_padded_len = 0;
    for (int i = 0; i < 8; ++i) {
        size_t padded_len = ((lengths[i] + 8) / 64 + 1) * 64;
        if (padded_len > max_padded_len) {
            max_padded_len = padded_len;
        }
    }

    // Allocate padded buffers for all inputs
    uint8_t* padded[8];
    for (int i = 0; i < 8; ++i) {
        padded[i] = (uint8_t*)_mm_malloc(max_padded_len, 32);
        if (!padded[i]) {
        ...
        // Copy input data
        memcpy(padded[i], bs[i], lengths[i]);
        
        // Add padding
        padded[i][lengths[i]] = 0x80;
        memset(padded[i] + lengths[i] + 1, 0, max_padded_len - lengths[i] - 1);
        
        // Append message length in bits (little-endian)
        uint64_t bit_length = lengths[i] * 8;
        memcpy(padded[i] + max_padded_len - 8, &bit_length, 8);
    }

    // Process all blocks in parallel
    size_t blocks = max_padded_len / 64;
    for (size_t j = 0; j < blocks; j++) {
        uint8_t* block_ptrs[8];
        for (int i = 0; i < 8; ++i) {
            block_ptrs[i] = padded[i] + j * 64;
        }
        ripemd160_avx2_transform(s, block_ptrs);
    }
